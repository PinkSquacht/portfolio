import React from "react";

const About = () => (
  <section className="w-100 py-4" style={{ background: '#ffe4ec' }}>
    <div className="container">
      <div className="card shadow-sm mx-auto" style={{ maxWidth: 1000, background: '#eafbe7' }}>
        <div className="card-body">
          <h2 className="mb-4">About Me</h2>
          <p>
            I’m Lloyd Bowar, a certified full-stack developer and IT consultant
            based in the U.S. I specialize in building modern, scalable web
            applications and helping individuals and small businesses solve
            real-world tech challenges.
          </p>
          <p>
            My expertise spans React, Node.js, Python, SQL, and more. I’m passionate
            about making technology accessible, whether it’s through step-by-step
            guides, security best practices, or custom software solutions. I thrive
            on problem-solving, continuous learning, and collaborating with others
            to deliver results that matter.
          </p>
          <p>
            When I’m not coding, I enjoy consulting for local businesses, exploring
            new tech trends, and sharing my knowledge to empower others. I’m
            available for freelance projects and collaboration—let’s
            build something great together!
          </p>
        </div>
      </div>
    </div>
  </section>
);

export default About;
